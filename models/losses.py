...
def multispectral loss(real, fake, n_fft_list=[2048, 1024, 512], hop_length_list=[512, 256, 128]):
  losses = []
  for n_fft, hop_length in zip(n_fft_list, hop_length_list):
    losses.append(spectral_loss(real, fake, n_fft, hop_length))

  reurn sum(losses) /len(losses)
...
def vq_vae_loss(real, fake, codes, beta=0.25, spec=True, spec_hp=1.0):
  l2_loss = F.mse_loss(fake, real)
  lat_loss = latent_loss(codes, beta=beta)
  spec_loss = 0
  if spec:
    spec_loss = spectral_loss(fake, real)
  vqvae_loss = [l2_loss, lat_loss, spec_hp*spec_loss]
  return vqvae_loss
